		...
		else if(args.length == 3)
		{
			if(args[0].equalsIgnoreCase("pay"))
			{
				...
				if(Economy.economyManager.hasEnoughMoney(player.getUniqueId(), inputs[0], inputs[1]))
				{
					...
					if(offlinePlayer.isOnline())
					{
						((Player)offlinePlayer).sendMessage(economy.getPrefix() + "Der Spieler " + economy.getHighlight() + player.getName() + " §7hat dir " + economy.getHighlight() + economy.calculator.toMoney(inputs[0], inputs[1]) + " §7überwiesen.");
					}
